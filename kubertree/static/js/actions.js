...
function actionsFor(node, caps, ctx) {
  ...
  if (kind === "Pod" || kind === "Container") {
    add(caps.logs, { label: "Logs", run: () => showLogs(node) });
    add(caps.exec, { label: "Exec shell", run: () => openExec(node) });
  }
  add(SCALABLE.has(kind) && caps.patch, { label: "Scale…", run: () => promptScale(node, ctx) });
  add(RESTARTABLE.has(kind) && caps.patch, { label: "Restart", run: () => doRestart(node, ctx) });
  add(UNDOABLE.has(kind) && caps.patch, { label: "Rollout undo", run: () => doUndo(node, ctx) });
  if (kind === "Node") {
    add(caps.cordon, { label: "Cordon", run: () => doCordon(node, ctx, true) });
    add(caps.cordon, { label: "Uncordon", run: () => doCordon(node, ctx, false) });
    add(caps.drain, { label: "Drain", danger: true, run: () => doDrain(node, ctx) });
  }
  add(kind !== "Cluster" && kind !== "Container" && caps.manifest,
    { label: "View YAML", run: () => showYaml(node) });
  add(caps.events && (node.data.namespace || kind === "Namespace"),
    { label: "Events", run: () => showEvents(node) });
  add(node.data.deletable && caps.delete,
    { label: "Delete (cascade)", danger: true, run: () => confirmDelete(node, ctx) });
  return actions;
}
