...
import kubertree._platform as _platform
import kubertree.api._actions as _api_actions
import kubertree.api._auth as _api_auth
import kubertree.api._exec as _api_exec
import kubertree.api._inspect as _api_inspect
from kubertree._state import state
from kubertree.auth._auth import ambient_clients, require_user_clients
from kubertree.k8s._client import ClusterClients, ClusterConnectionError
from kubertree.k8s._inventory import OwnerResolver, list_node_capacity, list_pods
from kubertree.k8s._metrics import fetch_pod_usage
...
async def lifespan(_: FastAPI):
    try:
        ...
        logger.info(
            "Connected: %s, metrics=%s",
            state.platform.kind,
            state.platform.metrics_available,
        )
...
def tree(group: str = "owner", clients: ClusterClients = Depends(require_user_clients)) -> dict:
    ...
    capacity = {
        name: {"cpu": cpu, "mem": mem} for name, (cpu, mem) in list_node_capacity(clients).items()
    }
    return {
