...
def capabilities(clients: ClusterClients, target: ResourceTarget) -> dict[str, bool]:
    ...
    if target.kind == "Node":
        ...
        caps["drain"] = caps["cordon"] and _allowed(clients, "create", "", "pods", None, "eviction")
    return caps
...
def _allowed(
    ...
    try:
        result = client.AuthorizationV1Api(clients.api_client).create_self_subject_access_review(
            body=review
        )
    except ApiException:
        return False
