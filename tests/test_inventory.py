...
from _inventory import OwnerResolver, _controller_ref
from _models import OwnerRef


def pod_owned_by(kind, name, uid, controller=True):
    owner = V1OwnerReference(
        api_version="apps/v1", kind=kind, name=name, uid=uid,
        controller=controller, block_owner_deletion=False,
    )
    return V1Pod(metadata=V1ObjectMeta(name="p", namespace="default", uid="pod-uid",
...
def test_controller_ref_prefers_controlling_owner():
    refs = [
        ...
        V1OwnerReference(api_version="apps/v1", kind="ReplicaSet", name="rs", uid="rs-uid", controller=True),
    ]
...
def test_owner_object_fetched_once_across_pods(monkeypatch):
    ...
    shared = lambda: pod_owned_by("ReplicaSet", "web-rs", "rs-uid")
    resolver.resolve_chain(shared())
    resolver.resolve_chain(shared())
