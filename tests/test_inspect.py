...
def test_read_logs_passes_parameters_through():
    ...
    assert core.log_args == {
        "name": "web-pod", "namespace": "default", "container": "app",
...
def test_list_events_filters_by_involved_object():
    event = SimpleNamespace(
        type="Warning", reason="BackOff", message="boom", count=3,
        last_timestamp=datetime(2026, 6, 1, tzinfo=timezone.utc), event_time=None)
    core = FakeCore([event])
