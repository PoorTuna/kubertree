...
from _nodeops import drain, set_cordon
from _resources import delete_resource
...
@router.post("/scale")
def scale_workload(request: ScaleRequest, clients: ClusterClients = Depends(require_user_clients)) -> dict:
    _run(lambda: scale(clients, request.target(), request.replicas))
    return {"scaled": True, "name": request.name, "replicas": request.replicas}


@router.post("/restart")
def restart_workload(request: TargetRequest, clients: ClusterClients = Depends(require_user_clients)) -> dict:
    _run(lambda: restart(clients, request.target()))
    return {"restarted": True, "name": request.name}


@router.post("/rollout-undo")
def undo_workload(request: TargetRequest, clients: ClusterClients = Depends(require_user_clients)) -> dict:
    _run(lambda: rollout_undo(clients, request.target()))
    return {"undone": True, "name": request.name}


@router.post("/cordon")
def cordon_node(request: CordonRequest, clients: ClusterClients = Depends(require_user_clients)) -> dict:
    _run(lambda: set_cordon(clients, request.name, request.on))
    return {"cordoned": request.on, "name": request.name}


@router.post("/drain")
def drain_node(request: NodeRequest, clients: ClusterClients = Depends(require_user_clients)) -> dict:
    evicted: list[str] = []
