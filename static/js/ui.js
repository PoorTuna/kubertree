...
import { escapeHtml, fmtCpu, fmtMem, kindIcon, efficiencyText } from "./format.js";
...
export function renderBreadcrumb(focus, onCrumb) {
  ...
  trail.forEach((node, index) => {
    ...
    crumb.textContent = `${kindIcon(node.data.kind)} ${node.data.name}`;
...
export function showTooltip(event, node, opts) {
  ...
  els.tooltip.innerHTML = `
    <h4>${kindIcon(node.data.kind)} ${escapeHtml(node.data.name)}</h4>
...
export function showPanel(node) {
  ...
  els.panel.innerHTML = `
    ...
    <span class="kind-tag">${kindIcon(node.data.kind)} ${escapeHtml(node.data.kind)}</span>
